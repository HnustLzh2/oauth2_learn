...
```sh
# 浏览器请求
http://localhost:9096/authorize?client_id=app_1&response_type=code&scope=all&state=xyz&redirect_uri=http://localhost:9093/cb
...
## SSO(单点登录)使用流程
1. 在某个客户端使用授权码authorize进行登录，登录成功之后，session中会保存此loggedUserID,并返回授权码在回调地址中
2. 使用另一个客户端再去换取授权码的时候，由于session保存了loggedUserID，所以可以不需要进行登录，直接拿到授权码
3. 使用授权码去换取token即可
